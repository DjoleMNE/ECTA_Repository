...
p.nGenes = 32;
p.popSize = 1;
nVar=p.nGenes;                % Number of Unknown (Decision) Variables
...
for g=1:MaxIt
    ...
    for i=1:lambda
        ...
        [value, matrix] = shape_calc_fitness(pop(i).Position, nacafoil, numEvalPts, p); 
        pop(i).Cost = value;
        pop(i).Foil = matrix;
    ...
    [value, matrix] = shape_calc_fitness(M(g+1).Position, nacafoil, numEvalPts, p); 
    M(g+1).Cost = value;
    M(g+1).Foil = matrix;
