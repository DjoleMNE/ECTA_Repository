function output = shape_CMA_ES(init_sigma, nacafoil, numEvalPts)
...
yInit = rand(p.popSize, p.nGenes) - 0.5;% initial parent vector 
sigmaInit = 1;                          % initial global mutation strength sigma 
sigmaMin = init_sigma;                  % CMA-ES stops when sigma is smaller than sigmaMin
...
%             OffspringIndividual.std(index)  = randn(1);
%             OffspringIndividual.w(index) = sigma * (SqrtCov(index, :) * OffspringIndividual.std(index)); % line (L1a)
%             temp = yParent(index) + OffspringIndividual.w(index);         % line (L1b)
%             while temp > 0.5 || temp < -0.5
%                 OffspringIndividual.std(index)  = 0.5 .* randn(1);
%                 OffspringIndividual.w(index) = sigma * (SqrtCov(index, :) * OffspringIndividual.std(index)); % line (L1a)
%                 temp = yParent(index) + OffspringIndividual.w(index);         % line (L1b)
%             end
