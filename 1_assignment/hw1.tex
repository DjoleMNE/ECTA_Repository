...
\textit{
 ...
 \item Crossover and Mutation perform two important roles in genetic algorithm.
 \item Crossover is a convergence operation which is intended to pull the population towards a local minimum/maximum.
 \item Mutation is a divergence operation. It is intended to occasionally break one or more members of a population out of a local minimum/maximum space and potentially discover a better minimum/maximum space.
 \item  As described in our result in the comparison graph, the aim is to bring the population to convergence.
 \item As shown in the graphs in our implementation , by performing only crossover, the fitness takes longer generations/time to reach the solution.
 \item By performing without mutation, it shows that the fitness reaches a particular level and gets stagnant/constant.
\end{itemize} 
\textbf{References}
\begin{itemize}
	\item https://www.researchgate.net/post/Is\_crossover\_a\_pillar\_of\_genetic\_algorithms
	\item https://www.researchgate.net/post/What\_is\_the\_role\_of\_mutation\_and\_crossover\_probability\_in\_Genetic\_algorithms
\end{itemize}
  }
%-------------------------%
...
\textit{
\color{blue}\begin{itemize}
	\item A Genetic Algorithm is supposed to have the possibility to explore the problem space. Mutation is a small step in exploring, whereas Crossover is potentially a larger step if crossover happens between individuals that are far apart. However, the larger the step is, the higher probability of producing a failure. When there is only crossover performed ,the algorithm selects genes from the individuals in the initial population and recombines them. The algorithm cannot create any new genes because there is no mutation.
	\item Crossover is a convergence operation which is intended to pull the population towards a local minimum/maximum.
\end{itemize}
\textbf{References}
\begin{itemize}
	\item https://www.researchgate.net/post/Is\_crossover\_a\_pillar\_of\_genetic\_algorithms
	\item https://www.researchgate.net/post/What\_is\_the\_role\_of\_mutation\_and\_crossover\_probability\_in\_Genetic\_algorithms
	\item https://stackoverflow.com/questions/17882992/genetic-algorithm-is-elitism-necessary-if-crossover-is-low
\end{itemize}
}
%-------------------------%
...
\textit{
\color{blue}\begin{itemize}
	\item Genetic Algorithms use randomness to create initial solutions, new individuals (via mutation and crossover operators) and even select individuals. However,  selection tends to favor the most fittest individuals. Also, genetic operators are not purely random but use randomness to (often slightly) change individuals.
	\item Whereas random search methods take much longer time to reach the solution.
\end{itemize}
}
%-------------------------%	
