...
% make graph
figure( hf );
hCart = patch( [state( 1 )-cart_width/2 state( 1 )-cart_width/2 state( 1 )+cart_width/2 state( 1 )+cart_width/2], ...
       [0 cart_height cart_height 0], ...
       [0.5 0.5 1.0] ); % the cart
hPole = patch( pole_vertices(:,1), pole_vertices(:,2), [.5 1.0 .5] ); % the pole
hHing = patch( hinge_X, hinge_Y, [0 0 0] ); % the hinge
hold on;
if action > 0
